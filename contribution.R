...
#import dataset
data <- read.csv("C:/Users/Adriani/Desktop/Middleton.csv", sep=";")

#reshape to wide format and manipulate data for meta-analysis
...
D=dataw
colnames(D) <- c("study","id","treat1","n1","r1","treat2","n2","r2")

#meta-analysis
comp<-paste(D$treat1,rep("vs",length(D$study)),D$treat2)

metaD <- metabin(c(D$r1), c(D$n1), c(D$r2), c(D$n2), 
                 sm="OR", method="I",allstudies=T) #pairwise meta-analysis

metaPairw<-metagen(metaD$TE, metaD$seTE, sm="OR",
                   comb.random=TRUE,byvar=comp)

#network meta-analysis
metaNetw<- netmeta(metaD$TE,metaD$seTE,D$treat1,D$treat2,
                   sm="OR",comb.random=TRUE, studlab=D$id)
...
tot.w<-apply(abs(krahn1$H),1,sum)
perc.H<-apply(abs(krahn1$H),2,"/",tot.w)
impD<-apply(abs(krahn1$H),2,sum)
tot.w.st<-apply(abs(krahn1$H.studies),1,sum)
perc.H.st<-apply(abs(krahn1$H.studies),2,"/",tot.w.st)

#final results
percentageContr=round(perc.H*100,1)
impD=round(impD/sum(impD)*100,1)
percentageContrStudies=round(perc.H.st*100,1)
