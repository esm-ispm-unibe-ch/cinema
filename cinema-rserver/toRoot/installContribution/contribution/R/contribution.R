...
if (type=="netwide_binary"){
  metaD <- metabin(c(D$r1), c(D$n1), c(D$r2), c(D$n2),
                   comb.random=(model=="random"),sm=sm,
                   method="I",allstudies=T) #pairwise meta-analysis

  comp<-paste(D$t1,rep("vs",length(D$id)),D$t2)

  metaPairw<-metagen(metaD$TE, metaD$seTE, sm=sm,
                     comb.random=(model=="random"),byvar=comp)
}
...
if(model=="random"){
  nma.krahn <- function(x, tau.preset=0){
    ...
    for (i in names(table(studies$comparison))){
      ...
      seTE.i <- studies$seTE[studies$comparison==i]
      #if (is.na(tau)){
      m1 <- metagen(TE.i, seTE.i, sm=x$sm, hakn=FALSE, method.tau="DL", tau.common=FALSE)
      #}
      #if (!is.na(tau)){
      #  m1 <- metagen(TE.i, seTE.i, sm=x$sm, hakn=FALSE, tau.preset=tau)
      #}
      ##
      direct$comparison[j] <- i
      direct$TE[j] <- m1$TE.random
      direct$seTE[j] <- m1$seTE.random
      ##
      if (sum(studies$comparison==i & !selmulti) > 0) {
        TE.i <- studies$TE[studies$comparison==i&studies$narms==2]
        seTE.i <- studies$seTE[studies$comparison==i&studies$narms==2]
        ...
        #}
        #if (!is.na(tau)){
        #  m2 <- metagen(TE.i, seTE.i, sm=x$sm, tau.preset=tau)
        #}
        ##
        direct$TE.2arm[j] <- m2$TE.random
        direct$seTE.2arm[j] <- m2$seTE.random
        direct$n.2arm[j] <- m2$k
      }
    }
...
V2bu=diag(direct$seTE^2,nrow=length(direct$seTE),ncol=length(direct$seTE))
H2bu <- krahn1$X.full %*% solve(t(X.obs2.design.2bu) %*% solve(V2bu) %*% X.obs2.design.2bu) %*% t(X.obs2.design.2bu) %*% solve(V2bu)

#rescale to percentages and derive total contribution
tot.w<-apply(abs(H2bu),1,sum)
perc.H<-apply(abs(H2bu),2,"/",tot.w)
impD<-apply(abs(H2bu),2,sum)
impD<-as.matrix(t(impD))
colnames(perc.H) <- colnames(impD) <- direct$comparison

tot.w.st<-apply(abs(krahn1$H.studies),1,sum)
...
return(list(colNames=colnames(percentageContr),rowNames=rownames(percentageContr),percentageContr=percentageContr,impD=impD, percentageContrStudies=percentageContrStudies,
